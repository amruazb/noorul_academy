...
export function buildPdfFilename({ date, className, period, from, to }) {
  const safeClass = (className || 'All-Classes').replace(/\s+/g, '-');
  if (period === 'weekly' || period === 'monthly') {
    const safeFrom = (from || date).replace(/-/g, '');
    const safeTo = (to || date).replace(/-/g, '');
    const label = period === 'weekly' ? 'Weekly' : 'Monthly';
    return `${label}-Report-${safeClass}-${safeFrom}-${safeTo}.pdf`;
  }
