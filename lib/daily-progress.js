...
export function buildStudyChartMessage({ date, students, entriesByStudentId, settings = defaultSettings }) {
  const lines = ['*STUDY CHART*', '', `🗓️Date: ${formatDisplayDate(date)}`, ''];
  const grouped = groupStudentsByCourse(students);

  for (const [course, courseStudents] of Object.entries(grouped)) {
    lines.push(`🔸🔸🔸 ${course.toUpperCase()} 🔸🔸🔸`, '');

    for (const student of courseStudents) {
      const entry = entriesByStudentId[String(student.id)] || emptyDailyEntry();
      lines.push(student.name, '');
      lines.push(`🔻 Attendance ${statusIcon(entry.attendance)}`);
      lines.push(`🔻 New Dars Practice Nazira ${statusIcon(entry.newDarsPractice)}`);
      lines.push(`🔻 Juz Dars Practice ${statusIcon(entry.juzDarsPractice)}`);
      lines.push(`🔻 Old Dars Practice ${statusIcon(entry.oldDarsPractice)}`);
